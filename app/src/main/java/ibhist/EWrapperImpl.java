...
public class EWrapperImpl implements EWrapper {
    ...
    //! [socket_init]
    public EWrapperImpl(ActionProvider actionProvider) {
    ...
    public void orderStatus(int orderId, String status, Decimal filled, Decimal remaining, double avgFillPrice, long permId, int parentId,
        ...
        actionProvider.findByType(OrderManagerAction.class)
                .ifPresent(action -> {
                    var s = new OrderManagerAction.OrderStatus(orderId, status, filled.value(), remaining.value(), avgFillPrice, permId, parentId, lastFillPrice, clientId, whyHeld, mktCapPrice);
                    action.onOrderStatus(s);
                });
    }
    //! [orderstatus]
    ...
    public void openOrder(int orderId, Contract contract, Order order, OrderState orderState) {
        ...
        // No req id for this message - unsolicited
        actionProvider.findByType(OrderManagerAction.class)
                .ifPresent(action -> action.onOpenOrder(order, contract));
    }
    //! [openorder]
    ...
    public void nextValidId(int orderId) {
        ...
        // No req id for this message - unsolicited
        actionProvider.findByType(NextOrderIdAction.class)
                .ifPresent(action -> action.process(orderId));
    }
    //! [nextvalidid]
    ...
    public void contractDetails(int reqId, ContractDetails contractDetails) {
        ...
        if (actionProvider.findById(reqId) instanceof ContractDetailsAction action) {
            action.onContractDetails(contractDetails);
        } else {
            logNotFound(reqId);
        }
    }
    ...
    public void historicalData(int reqId, Bar bar) {
        if (actionProvider.findById(reqId) instanceof HistoricalDataAction action) {
            action.onHistoricalData(bar);
        } else {
            logNotFound(reqId);
        }
    }
    ...
    //! [scannerdata]
    @Override
    public void scannerData(int reqId, int rank, ContractDetails contractDetails, String distance, String
            benchmark, String projection, String legsStr) {
    ...
    //! [realtimebar]
    @Override
    public void realtimeBar(int reqId, long time, double open, double high, double low, double close, Decimal
            volume, Decimal wap, int count) {
        if (actionProvider.findById(reqId) instanceof RealTimeBarsAction action) {
            var dt = LocalDateTime.ofInstant(new Date(time * 1000).toInstant(), ZoneId.of("UTC"));
            action.onRealtimeBar(new RealTimeBar(dt, open, high, low, close, volume.longValue(), wap.longValue()));
        } else {
            logNotFound(reqId);
        }
    }
    ...
    //! [accountupdatemulti]
    @Override
    public void accountUpdateMulti(int reqId, String account, String modelCode, String key, String value, String
            currency) {
    ...
    @Override
    public void securityDefinitionOptionalParameter(int reqId, String exchange, int underlyingConId, String
                                                            tradingClass, String multiplier,
...
//		log.info("historicalDataUpdate reqId = " + reqId);
        if (actionProvider.findById(reqId) instanceof HistoricalDataAction action) {
            action.onHistoricalDataUpdate(bar);
        } else {
            logNotFound(reqId);
        }
    }
    ...
    //! [tickbytickalllast]
    @Override
    public void tickByTickAllLast(int reqId, int tickType, long time, double price, Decimal size, TickAttribLast
                                          tickAttribLast,
    ...
    //! [tickbytickbidask]
    @Override
    public void tickByTickBidAsk(int reqId, long time, double bidPrice, double askPrice, Decimal bidSize, Decimal
                                         askSize,
    ...
    //! [historicalSchedule]
    @Override
    public void historicalSchedule(int reqId, String startDateTime, String endDateTime, String
            timeZone, List<HistoricalSession> sessions) {
    ...
    //! [execDetailsEnd]
    @Override
    public void execDetailsEndProtoBuf(ExecutionDetailsEndProto.ExecutionDetailsEnd
